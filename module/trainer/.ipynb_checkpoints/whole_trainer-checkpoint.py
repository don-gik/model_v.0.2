...
class AxialPrediction(nn.Module):
    def __init__(self,
        ...
        self.future_proj = nn.Conv3d(embedDim, embedDim*prediction, kernel_size=1, bias=True)

    def forward(self, x):                      # x: [B,T,C,H,W]
        z_list = [self.encoder(x[:, t:t+1]).unsqueeze(1) for t in range(x.size(1))]
        z = torch.cat(z_list, 1)               # [B,T,Cz,H',W']
        z = self.transformer(z)                # [B,T,Cz,H',W']

        z_last = z[:, -1:].permute(0,2,1,3,4)  # [B,Cz,1,H',W']
        fut = self.future_proj(z_last)         # [B,Cz*P,1,H',W']
        B, _, _, Hp, Wp = fut.shape
        fut = fut.view(B, self.prediction, -1, Hp, Wp)  # [B,P,Cz,H',W']

        outs = [self.decoder(fut[:, p]) for p in range(self.prediction)]
        return torch.stack(outs, 1)            # [B,P,C,H,W]
...
def run():
    ...
    main(
        ...
        checkpointDir = "./models/whole_oni",
        lastCheckpoint = "./models/whole_oni/axial_attention_mlp+1.pth",
