...
class AxialAttention(nn.Module):
    ...
    def forward(self, x, relativeBias):    # x: [B, T, C, H, W]
        ...
        use_causal = (self.axis == "time")
        if use_causal:
            neg_inf = -1e4 if q.dtype in (torch.float16, torch.bfloat16) else torch.finfo(q.dtype).min
            causal = torch.triu(torch.full((L, L), neg_inf, device=q.device, dtype=q.dtype), 1)  # [L, L]
